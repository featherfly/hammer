 ...
 * @author zhongj
 */
public class SqlResultSet implements cn.featherfly.hammer.mapping.ResultSet {
    ...
    public <T extends Enum<T>> T getEnum(int index, Class<T> type) {
        try {
            ...
            // TODO 标准化
            throw new SqldbHammerException("can not convert type " + proxy.getType() + " to enum " + type.getName());
        } catch (SQLException e) {
            throw new SqldbHammerException(e);
        }
    }
