...
public abstract class RepositorySqlRelation<R extends RepositorySqlRelation<R, B>, B extends SqlBuilder> {
    ...
    public R addFilterable(int sourceIndex, String sourceField, String joinRepository, String joinField) {
        return addFilterable(sourceIndex, sourceField, joinRepository, joinField);
    }
    ...
    public R addFilterable(int sourceIndex, String sourceField, String joinRepository, String joinRepositoryAlias,
        ...
        switch (index) {
            case 0:
                RepositoryRelation eqm = createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField);
                repositoryFilterableTuple.set0(eqm);
                initBuilder(eqm);
                break;
            case 1:
                repositoryFilterableTuple.set1(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 2:
                repositoryFilterableTuple.set2(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 3:
                repositoryFilterableTuple.set3(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 4:
                repositoryFilterableTuple.set4(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 5:
                repositoryFilterableTuple.set5(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 6:
                repositoryFilterableTuple.set6(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 7:
                repositoryFilterableTuple.set7(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            case 8:
                repositoryFilterableTuple.set8(createEntityRelationMapping(sourceIndex, sourceField, joinRepository,
                        joinRepositoryAlias, joinField));
                break;
            default:
                break;
    ...
    public R join(int sourceIndex, String sourceField, String joinRepository) {
        ...
        throw new SqldbHammerException(
                Strings.format("only support one privary key, but more than one privary key found {0}", pkList.size()));
    }
    ...
    public R join(int sourceIndex, String sourceField, String joinRepository, String joinField) {
        ...
        SqlSelectJoinOnBasicBuilder selectJoinOnBasicBuilder = join0(jerm.getRepositoryAlias(), jerm.getField(),
                erm.getRepository(), erm.getRepositoryAlias(), erm.getField());
     ...
     */
    protected RepositoryRelation createEntityRelationMapping(int sourceIndex, String sourceField, String joinRepository,
