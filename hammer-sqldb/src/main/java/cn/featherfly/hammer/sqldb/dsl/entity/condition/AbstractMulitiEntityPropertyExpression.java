...
public abstract class AbstractMulitiEntityPropertyExpression<E, C extends ConditionExpression,
    ...
    public PropertyMapping<?> getPropertyMapping(Object value) {
        if (propertyList.size() == 1) {
            ClassMapping<?, JdbcPropertyMapping> classMapping = expression.getClassMapping(index.get());
            ...
            for (int i = 0; i < propertyList.size() - 1; i++) {
                pm = getSubPropertyMapping(propertyList.get(i), propertyList.get(i + 1), value);
            }
            return pm;
        } else {
    ...
    private PropertyMapping<?> getSubPropertyMapping(Serializable property, Serializable subProperty, Object value) {
        ...
        if (pm.getMode() == Mode.EMBEDDED) {
            ...
            if (spm != null) {
                ...
                spm = cm.getPropertyMapping(pn);
                if (spm != null) {
                    queryRelation.join(Join.LEFT_JOIN, currentIndex, pm.getPropertyName(), cm, false);
