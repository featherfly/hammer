 ...
 */
public abstract class AbstractMulitiRepositorySqlConditionsGroupExpression<C extends GroupExpression<C, L>,
        L extends GroupEndExpression<C, L>, C2 extends ConditionConfig<C2>, S extends RepositorySqlRelation<S, B>,
        B extends SqlBuilder> extends AbstractSqlConditionsGroupExpression<C, L, C2>
        implements InternalMulitiCondition<L>, MulitiRepositoryExpression {
    // FIXME 使用AbstractMulitiRepositorySqlConditionExpression代替
     ...
     */
    protected AbstractMulitiRepositorySqlConditionsGroupExpression(L parent, Dialect dialect, String repositoryAlias,
            S repositoryRelation, C2 conditionConfig) {
    ...
    public <V> L ba(AtomicInteger index, String name, V min, V max, Predicate<?> ignoreStrategy) {
        return (L) addCondition(new SqlConditionExpressionBuilder(getDialect(), name,
                new FieldValueOperator[] { getFieldValueOperator(min), getFieldValueOperator(max) },
