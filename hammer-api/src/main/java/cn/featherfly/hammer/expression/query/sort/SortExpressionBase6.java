...
public interface SortExpressionBase6<S extends SortedExpression<S>> extends SortExpressionBase5<S> {
    ...
    default S asc6(Field... fields) {
        ...
        return asc6(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S asc6(AliasField... fields) {
        ...
        return asc6(Arrays.stream(fields).map(AliasField::getAliasOrName)
    ...
    default S desc6(Field... fields) {
        ...
        return desc6(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S desc6(AliasField... fields) {
        ...
        return desc6(Arrays.stream(fields).map(AliasField::getAliasOrName)
                .toArray(num -> ArrayUtils.create(String.class, num)));
    }
}