...
public interface SortExpressionBase2<S extends SortedExpression<S>> extends SortExpression<S> {
    ...
    default S asc2(Field... fields) {
        ...
        return asc2(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S asc2(AliasField... fields) {
        ...
        return asc2(Arrays.stream(fields).map(AliasField::getAliasOrName)
    ...
    default S desc2(Field... fields) {
        ...
        return desc2(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S desc2(AliasField... fields) {
        ...
        return desc2(Arrays.stream(fields).map(AliasField::getAliasOrName)
                .toArray(num -> ArrayUtils.create(String.class, num)));
    }
}