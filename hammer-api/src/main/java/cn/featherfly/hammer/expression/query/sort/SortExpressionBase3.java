...
public interface SortExpressionBase3<S extends SortedExpression<S>> extends SortExpressionBase2<S> {
    ...
    default S asc3(Field... fields) {
        ...
        return asc3(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S asc3(AliasField... fields) {
        ...
        return asc3(Arrays.stream(fields).map(AliasField::getAliasOrName)
    ...
    default S desc3(Field... fields) {
        ...
        return desc3(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S desc3(AliasField... fields) {
        ...
        return desc3(Arrays.stream(fields).map(AliasField::getAliasOrName)
                .toArray(num -> ArrayUtils.create(String.class, num)));
    }
}