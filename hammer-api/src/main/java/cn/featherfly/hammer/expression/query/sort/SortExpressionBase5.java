...
public interface SortExpressionBase5<S extends SortedExpression<S>> extends SortExpressionBase4<S> {
    ...
    default S asc5(Field... fields) {
        ...
        return asc5(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S asc5(AliasField... fields) {
        ...
        return asc5(Arrays.stream(fields).map(AliasField::getAliasOrName)
    ...
    default S desc5(Field... fields) {
        ...
        return desc5(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S desc5(AliasField... fields) {
        ...
        return desc5(Arrays.stream(fields).map(AliasField::getAliasOrName)
                .toArray(num -> ArrayUtils.create(String.class, num)));
    }
}