...
public interface SortExpressionBase4<S extends SortedExpression<S>> extends SortExpressionBase3<S> {
    ...
    default S asc4(Field... fields) {
        ...
        return asc4(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S asc4(AliasField... fields) {
        ...
        return asc4(Arrays.stream(fields).map(AliasField::getAliasOrName)
    ...
    default S desc4(Field... fields) {
        ...
        return desc4(Arrays.stream(fields).map(Field::name).toArray(num -> ArrayUtils.create(String.class, num)));
    }
    ...
    default S desc4(AliasField... fields) {
        ...
        return desc4(Arrays.stream(fields).map(AliasField::getAliasOrName)
                .toArray(num -> ArrayUtils.create(String.class, num)));
    }
}