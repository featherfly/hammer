...
public interface RepositoryNotBetweenExpressionBase5<C extends ConditionExpression, L extends LogicExpression<C, L>>
    ...
    NotBetweenSupplierExpression5<C, L> {
    ...
    default <N extends Number> L nba5(SerializableNumberSupplier<N> property, N min, N max) {
        return nba5(LambdaUtils.getLambdaPropertyName(property), min, max);
    }

    @Override
    default <N extends Number> L nba5(SerializableNumberSupplier<N> property, N min, N max,
        BiPredicate<N, N> ignoreStrategy) {
        return nba5(LambdaUtils.getLambdaPropertyName(property), min, max, ignoreStrategy);
    }

    @Override
    default <D extends Date> L nba5(SerializableDateSupplier<D> property, D min, D max) {
        return nba5(LambdaUtils.getLambdaPropertyName(property), min, max);
    }
    ...
    @Override
    default L nba5(SerializableStringSupplier property, String min, String max) {
        return nba5(LambdaUtils.getLambdaPropertyName(property), min, max);
    }
