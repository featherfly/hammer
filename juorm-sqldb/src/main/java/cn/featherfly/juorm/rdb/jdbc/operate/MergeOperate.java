...
public class MergeOperate<T> extends AbstractOperate<T> {
    ...
    private String getDynamicSql(T entity,
            Map<Integer, String> propertyPositions, boolean onlyNull) {
        ...
        for (PropertyMapping propertyMapping : classMapping
            ...
            if (propertyMapping.getPropertyMappings().isEmpty()) {
                ...
                if (propertyMapping.isPrimaryKey()) {
                    ...
                    columnNum = set(entity, propertyMapping, updateSql,
                            propertyPositions, columnNum);
                }
            } else {
                for (PropertyMapping subPropertyMapping : propertyMapping
                        .getPropertyMappings()) {
                    if (checkNullOrEmpty(entity, subPropertyMapping,
                            onlyNull)) {
                        continue;
                    }
                    if (subPropertyMapping.isPrimaryKey()) {
                        ...
                        columnNum = set(entity, subPropertyMapping, updateSql,
                                propertyPositions, columnNum);
                    }
                }
            }
        ...
        int pkNum = 0;
        updateSql.append(jdbc.getDialect().getKeywords().where())
                .append(Chars.SPACE);
    ...
    private boolean checkNullOrEmpty(T entity, PropertyMapping propertyMapping,
            boolean onlyNull) {
    ...
    private int set(T entity, PropertyMapping propertyMapping,
            StringBuilder updateSql, Map<Integer, String> propertyPositions,
            int columnNum) {
        updateSql
                .append(jdbc.getDialect()
        ...
        propertyPositions.put(columnNum,
                ClassMappingUtils.getPropertyAliasName(propertyMapping));
        return columnNum;
    }
