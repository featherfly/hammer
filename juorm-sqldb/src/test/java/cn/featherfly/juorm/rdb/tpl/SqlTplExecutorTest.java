...
public class SqlTplExecutorTest extends JdbcTestBase {
    ...
    void testNumberValue() {
        Integer avg = executor.intValue("selectAvg",
                new HashChainMap<String, Object>());
        System.out.println("avg(age) = " + avg);
        assertTrue(avg > 20);

        avg = executor.intValue("selectAvg2",
                new HashChainMap<String, Object>().putChain("age", 40));
        System.out.println("avg(age) = " + avg);
        assertTrue(avg > 40);
    ...
    void testStringValue() {
        String str = executor.stringValue("selectString",
                new HashChainMap<String, Object>());
        System.out.println("selectString = " + str);
        assertEquals(str, "yufei");

        str = executor.stringValue("selectString2",
                new HashChainMap<String, Object>().putChain("id", 2));
        System.out.println("selectString = " + str);
    ...
    void testSingle() {
        ...
        User u2 = executor.single("user@selectByUsernameAndPassword",
                User.class,
                new HashChainMap<String, Object>()
        ...
        u2 = executor.single("/tpl/user@selectByUsernameAndPassword",
                User.class,
                new HashChainMap<String, Object>()
                        .putChain("username", username)
                        .putChain("password", password));
    ...
    void testUserList() {
        Integer age = 5;
        List<User> users = executor.list("user@selectUser", User.class,
                new HashChainMap<String, Object>());
        assertTrue(users.size() > 0);

        users = executor.list("user@selectByAge", User.class,
    ...
    void testUserList2() {
        ...
        List<User> users = executor.list("user@selectConditions", User.class,
                new HashChainMap<String, Object>());
        assertTrue(users.size() > 0);

        users = executor.list("user@selectConditions", User.class,
    ...
    void testUserPagination() {
        ...
        PaginationResults<User> userPaginationResults = executor.pagination(
                "user@selectConditions", User.class,
                new HashChainMap<String, Object>(), start, limit);
        assertTrue(userPaginationResults.getResultSize() == 3);

        userPaginationResults = executor.pagination("user@selectConditions",
                User.class,
                new HashChainMap<String, Object>().putChain("minAge", minAge)
                        .putChain("maxAge", maxAge)
                        .putChain("username", username1 + "%"),
    ...
    void testUserInfoList() {
        List<UserInfo> uis = executor.list("user_info@select", UserInfo.class,
                new HashChainMap<String, Object>());
    ...
    void testUserInfoList2() {
        List<UserInfo> uis = executor.list("user_info@select2", UserInfo.class,
                new HashChainMap<String, Object>());
        assertTrue(uis.size() > 0);
        uis.forEach(ui -> {
            System.out.println(ui);
    ...
    void testWithTemplate() {
        PaginationResults<Role> uis = executor.pagination(
                "role@selectWithTemplate", Role.class,
    ...
    void testWithTemplate2() {
        PaginationResults<Role> uis = executor.pagination(
                "role@selectWithTemplate2", Role.class,
                new HashChainMap<String, Object>(), 0, 10);
        assertTrue(uis.getResultSize() > 0);
        System.out.println("result size:" + uis.getResultSize());
        uis.getPageResults().forEach(ui -> {
    ...
    void testWithTemplate3() {
        PaginationResults<Role> uis = executor.pagination(
                "role@selectWithTemplate3", Role.class,
                new HashChainMap<String, Object>(), 0, 10);
        assertTrue(uis.getResultSize() > 0);
        System.out.println("result size:" + uis.getResultSize());
        uis.getPageResults().forEach(ui -> {
    ...
    @Test
    void testMapList() {
        List<Map<String, Object>> uis = executor.list("user_info@select2",
                new HashChainMap<String, Object>(), 0, 10);
        assertTrue(uis.size() > 0);
    ...
    void testDuplicateExceptionOneFile() {
        ...
        executor.list("duplicate@select", UserInfo.class,
                new HashChainMap<String, Object>());
    }
    ...
    void testDuplicateExceptionMulitiFile() {
        // 配置会读取最下面的同名executeId
        executor.list("select", UserInfo.class,
                new HashChainMap<String, Object>());
    }
}
