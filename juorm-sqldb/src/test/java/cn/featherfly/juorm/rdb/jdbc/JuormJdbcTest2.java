...
public class JuormJdbcTest2 extends JdbcTestBase {
    ...
    public void testDeleteMulityPrimaryKey() {
        ...
        UserRole ur = juorm.query(UserRole.class).where().eq("role_id", 11).and().eq("user_id", 11)
    ...
    public void testQuery() {
        List<Role> roles = juorm.query(Role.class).where().gt("id", 5).and().le("id", 10).list(Role.class);
        for (Role role : roles) {
            System.out.println(role);
        ...
        Integer id = 10;
        Role r = juorm.query(Role.class).where().eq("id", id).single(Role.class);
    ...
    public void testQueryLimit() {
        List<Role> roles = juorm.query(Role.class).where().gt("id", 5).and().le("id", 10).limit(2).list(Role.class);
        ...
        roles = juorm.query(Role.class).where().gt("id", 5).and().le("id", 10).limit(2, 3).list(Role.class);
