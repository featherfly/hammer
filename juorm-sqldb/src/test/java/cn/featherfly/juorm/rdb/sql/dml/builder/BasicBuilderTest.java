...
public class BasicBuilderTest {
    ...
    void testSqlSelectBasicBuilder() {
        SqlSelectBasicBuilder builder = new SqlSelectBasicBuilder(
                Dialects.MYSQL, "user", "u");
        builder.addSelectColumns("username", "mobile", "age", "sex");
        System.out.println(builder.build());
        assertEquals(
                "SELECT u.`username`, u.`mobile`, u.`age`, u.`sex` FROM `user` u",
                builder.build());

        builder = new SqlSelectBasicBuilder(Dialects.MYSQL, "user");
        builder.addSelectColumns("username", "mobile", "age", "sex");
        System.out.println(builder.build());
        assertEquals("SELECT `username`, `mobile`, `age`, `sex` FROM `user`",
                builder.build());
        ...
        System.out.println(builder.build());
        assertEquals("SELECT COUNT(u.`user_id`) FROM `user` u",
                builder.build());
    ...
    void testSqlOrderByBasicBuilder() {
        ...
        System.out.println(builder.build());
        assertEquals(" ORDER BY `name`, `age` ASC, `sex` DESC",
                builder.build());
        ...
        builder = new SqlOrderByBasicBuilder(Dialects.MYSQL);
        builder.addAsc("name").addDesc("sex").addDesc("mobile").addAsc("age")
                .addAsc("no");
        System.out.println(builder.build());
        assertEquals(
                " ORDER BY `name` ASC, `sex`, `mobile` DESC, `age`, `no` ASC",
                builder.build());

        String alias = "u";
        String alias2 = "p";
        builder = new SqlOrderByBasicBuilder(Dialects.MYSQL);
        builder.addAsc("name", alias).addDesc("sex", alias)
                .addDesc("mobile", alias2).addAsc("age", alias2)
                .addAsc("no", alias);
        System.out.println(builder.build());
        assertEquals(
                " ORDER BY u.`name` ASC, u.`sex`, p.`mobile` DESC, p.`age`, u.`no` ASC",
                builder.build());
    }
    ...
    void testSqlUpdateSetBasicBuilder() {
        ...
        SqlUpdateSetBasicBuilder builder = new SqlUpdateSetBasicBuilder(
                Dialects.MYSQL, "user");
        builder.setValue("name", name).setValue("age", age).setValue("sex", sex)
                .setValue("pwd", pwd);
        System.out.println(builder.build());
        System.out.println(builder.getParams());
        assertEquals(
                "UPDATE `user` SET `name` = ?, `age` = ?, `sex` = ?, `pwd` = ?",
                builder.build());
        ...
        System.out.println(builder.getParams());
        assertEquals(
                "UPDATE `user` u SET u.`name` = ?, u.`age` = ?, u.`sex` = ?, u.`pwd` = ?",
                builder.build());
        assertEquals(params, builder.getParams());
    }
    ...
    void testSqlJoinBasicBuilder() {
        SqlJoinOnBasicBuilder builder = new SqlJoinOnBasicBuilder(
                Dialects.MYSQL, "user", "uid", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(), "JOIN `user` ON `uid` = `user_id`");

        builder = new SqlJoinOnBasicBuilder(Dialects.MYSQL, Join.LEFT_JOIN,
                "user", "uid", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(), "LEFT JOIN `user` ON `uid` = `user_id`");

        builder = new SqlJoinOnBasicBuilder(Dialects.MYSQL, Join.RIGHT_JOIN,
                "user", "uid", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(), "RIGHT JOIN `user` ON `uid` = `user_id`");

        builder = new SqlJoinOnBasicBuilder(Dialects.MYSQL, Join.FULL_JOIN,
                "user", "uid", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(), "FULL JOIN `user` ON `uid` = `user_id`");

        builder = new SqlJoinOnBasicBuilder(Dialects.MYSQL, "user", "u", "uid",
                "r", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(), "JOIN `user` u ON u.`uid` = r.`user_id`");

        builder = new SqlJoinOnBasicBuilder(Dialects.MYSQL, Join.LEFT_JOIN,
                "user", "u", "uid", "r", "user_id");
        System.out.println(builder.build());
        assertEquals(builder.build(),
                "LEFT JOIN `user` u ON u.`uid` = r.`user_id`");

    }

    @Test
    void testSqlDeleteFromBasicBuilder() {
        SqlDeleteFromBasicBuilder builder = new SqlDeleteFromBasicBuilder(
                Dialects.MYSQL, "user");
        System.out.println(builder.build());
