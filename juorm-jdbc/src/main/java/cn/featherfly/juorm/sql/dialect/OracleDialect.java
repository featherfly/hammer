...
public class OracleDialect extends AbstractDialect {
	 ...
	 */
	@Override
	public Object[] getPaginationSqlParameter(Object[] params,
			int start, int limit) {
	...
	private String getPaginationSql(String sql, int start, boolean isParamNamed) {
		...
		if (isParamNamed) {
			if (start > 0) {
				pagingSelect.append(
						String.format(" ) row_ ) where rownum_ > :%s and rownum_ <= :%s ", START_PARAM_NAME, LIMIT_PARAM_NAME)
						);
			} else {
				pagingSelect.append(
						String.format(") where rownum <= :%s", LIMIT_PARAM_NAME)
				);
			}
		} else {
			if (start > 0) {
	...
	public String convertValueToSql(Object value, int sqlType) {
		...
		if (value == null) {
			...
			case Types.NUMERIC:
			case Types.TINYINT:
				sqlPart.append(value); break;
			case Types.DATE:
			case Types.TIME:
			case Types.TIMESTAMP:
				if (value instanceof Date) {
					sqlPart.append("to_date('")
						.append(DateUtils.formart((Date) value, "yyyy-MM-dd HH:mm:ss"))
						.append("' , 'yyyy-mm-dd hh24:mi:ss')");
				} else {
					sqlPart.append("to_date('").append(value).append("' , 'yyyy-mm-dd hh24:mi:ss')"); break;
				}
				break;
			default:
				sqlPart.append("'").append(SqlUtils.transferStringForSql(value.toString())).append("'"); break;
			}
		}
