...
public abstract class AbstractSqlConditionGroupExpression<
        C extends ConditionGroupExpression<C, L>,
        L extends ConditionGroupLogicExpression<C, L>>
        implements ConditionGroupExpression<C, L>,
        ConditionGroupLogicExpression<C, L>, SqlBuilder, ParamedExpression {
     ...
     *            queryAlias
     */
    protected AbstractSqlConditionGroupExpression(Dialect dialect, L parent,
            String queryAlias) {
        ...
        if (conditions.size() > 0) {
            ...
            if (last instanceof SqlLogicExpression) {
                throw new BuilderException(
                        ((SqlLogicExpression) last).getLogicOperator()
                                + " 后没有跟条件表达式");
            }
        }
        ...
        for (Expression expression : conditions) {
            ...
            if (StringUtils.isNotBlank(condition)) {
                ...
                if (availableExpressions.size() > 0) {
                    Expression pre = availableExpressions
                            .get(availableExpressions.size() - 1);
                    if (pre instanceof SqlLogicExpression) {
                        availableExpressions
                                .remove(availableExpressions.size() - 1);
                        availableConditions
                                .remove(availableConditions.size() - 1);
                    }
                }
            }
        }
    ...
    private Object addCondition(Expression condition) {
        if (previousCondition != null) {
            if (previousCondition.getClass().isInstance(condition)) {
                throw new BuilderException(
                        "语法错误，连续相同类型的表达式：" + condition.getClass().getName());
            }
        }
    ...
    public L co(String name, String value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.CO, queryAlias));
    }
    ...
    public L ew(String name, String value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.EW, queryAlias));
    }
    ...
    public L eq(String name, Object value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.EQ, queryAlias));
    }
    ...
    public <N extends Number> L ge(String name, Number value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.GE, queryAlias));
    }
    ...
    public <N extends Number> L gt(String name, Number value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.GT, queryAlias));
    }
    ...
    public L in(String name, Object value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.IN, queryAlias));
    }
    ...
    public L inn(String name) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                null, QueryOperator.INN, queryAlias));
    }
    ...
    public L isn(String name) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                null, QueryOperator.ISN, queryAlias));
    }
    ...
    public <N extends Number> L le(String name, Number value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.LE, queryAlias));
    }
    ...
    public <N extends Number> L lt(String name, Number value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.LT, queryAlias));
    }
    ...
    public L ne(String name, Object value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.NE, queryAlias));
    }
    ...
    public L nin(String name, Object value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.NIN, queryAlias));
    }
    ...
    public L sw(String name, String value) {
        return (L) addCondition(new SqlConditionExpressionBuilder(dialect, name,
                value, QueryOperator.SW, queryAlias));
    }
    ...
    @Override
    public C and() {
        return (C) addCondition(
                new SqlLogicOperatorExpressionBuilder(LogicOperator.AND));
    }
    ...
    public C or() {
        return (C) addCondition(
                new SqlLogicOperatorExpressionBuilder(LogicOperator.OR));
    }
}
