...
public class MySqlDialect extends AbstractDialect {
	...
	private String getPaginationSql(String sql, int start, boolean isParamNamed) {
		...
		if (isParamNamed) {
			if (start > 0) {
				pagingSelect.append(String.format(
						" LIMIT :%s,:%s", START_PARAM_NAME, LIMIT_PARAM_NAME));
			} else {
		...
		if (value == null) {
			...
			case Types.TIMESTAMP:
				if (value instanceof Date) {
					sqlPart.append("'").append(
							DateUtils.formart((Date) value, "yyyy-MM-dd HH:mm:ss")).append("'");
				} else {
					sqlPart.append("'").append(value).append("'"); break;
				}
				break;
			default:
